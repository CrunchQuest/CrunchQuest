...
import com.crunchquest.android.domain.utility.handlers.UserHandler
...
class ProfileSettingActivity : AppCompatActivity() {
        ...
        if (selectedPhotoUri != null) {
            ...
            ref.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    val userImageUrl: String = user!!.profilePicture.toString()
    ...
    private fun chooseImage() {
        ...
        startActivityForResult(intent, SignUpActivity.IMAGE_PICK_CODE)
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        when (requestCode) {
            SignUpActivity.IMAGE_PICK_CODE -> {
    ...
    private fun deleteThePreviousImageFromFirebaseDatabase() {
        ...
        ref.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val user = snapshot.getValue(User::class.java)
                if (user!!.profilePicture != SignUpActivity.DEFAULT_IMG_URL) {
                    val storageRef = FirebaseStorage.getInstance().getReferenceFromUrl(user.profilePicture!!)
                    storageRef.delete()
                }
    ...
    private fun readUserData() {
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                ...
                emailAddressProfileEditText.setText(userData.email)
                mobileNumberEditText.setText(userData.phoneNumber)
                bioEditText.setText(userData.bio)
                //Picasso is a caching manager for the image
                Picasso.get().load(userData.profilePicture).into(profileImageView)
            }
