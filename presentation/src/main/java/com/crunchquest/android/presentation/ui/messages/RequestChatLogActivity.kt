...
import com.crunchquest.android.presentation.ui.components.groupie_views.ChatFromItem
import com.crunchquest.android.presentation.ui.components.groupie_views.ChatToItem
...
class RequestChatLogActivity : AppCompatActivity() {
    ...
    private var request: com.crunchquest.android.domain.model.ServiceRequest? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        request = intent.getSerializableExtra(REQUEST_EXTRA_TAG) as? com.crunchquest.android.domain.model.ServiceRequest
        //Map everything here
    ...
    private fun messagesListener() {
        ...
        ref.addChildEventListener(object : ChildEventListener {
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                val message = snapshot.getValue(com.crunchquest.android.domain.entities.Message::class.java)
                if (message != null) {
                    //If it's the message of the person in conversation with
                    if (message.senderId == FirebaseAuth.getInstance().currentUser!!.uid) {
                        val currentUser = MainActivity.currentUser ?: return
                        adapter.add(ChatFromItem(message.content!!, currentUser, message.timestamp!!))
                    }
                    //If it's a from you.
                    else {
                        adapter.add(ChatToItem(message.content!!, toUser!!, message.timestamp!!))
                    }
                }
    ...
    private fun performSendMessage() {
        ...
        dataRef.addListenerForSingleValueEvent(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val message = com.crunchquest.android.domain.model.RequestMessage(
                    uid = ref.key,
                    text = text,
                    fromId = fromId,
                    toId = toId,
